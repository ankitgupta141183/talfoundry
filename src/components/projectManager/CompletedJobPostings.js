...
export default class CompletedJobPostings extends Component {
  ...
  renderData = () => {
    let {completedJobsData} = this.props
    const {activePage} = this.state;
    let onlyNewJobs = JSON.parse(JSON.stringify((completedJobsData))).splice( activePage === 1 ? 0 : ((activePage - 1)*itemsCountPerPage), itemsCountPerPage)
    let some = [];
    onlyNewJobs.map((job, idx) => {
      some.push(
        ...
        <div className="my-post-title-sec list-view-strip-box" key={job.id}>
          ...
          <div className="tf_my_aws job-details-container">
            <div className="col-md-11 col-sm-11 col-xs-10 ">
              <Link to={`/job/${job.uuid || job.id}/1`}><h4>{job.job_title}
              {
    ...
    render(){
        ...
        return(
            <div>
                {completedJobsData && completedJobsData.length > 0 ? this.renderData() : ''}

                        {
                          (!_.isEmpty(completedJobsData) && completedJobsData.length > 0) ? (
                            <div className="">
                                <div className="col-md-12">
                                  <Pagination
                                    activePage={this.state.activePage}
                                    itemsCountPerPage={itemsCountPerPage}
                                    totalItemsCount={completedJobsData.length}
